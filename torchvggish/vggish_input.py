...
def resample(sample_rate,waveform):
    """Resample.
...
def wavfile_to_examples2(wav_file, return_tensor=True):
    """Convenience wrapper around waveform_to_examples() for a common WAV format.
  ...
  """
    # Load audio
    wav_data, sar = sf.read(wav_file, dtype='int16')


    (waveform, _) = librosa.core.load(wav_file, sr=sar, mono=True)
    if waveform.size%2==1:
        waveform=waveform[:waveform.size-1]
    waveform = waveform[None, :]    # (1, audio_length)

    waveform = move_data_to_device(waveform)
