...
class Wavegram_Logmel_Cnn14(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        ...
        self.conv_block3 = ConvBlock(in_channels=128, out_channels=256)
        self.conv_block4 = ConvBlock(in_channels=256, out_channels=512)
        self.conv_block5 = ConvBlock(in_channels=512, out_channels=512)
        # self.conv_block6 = ConvBlock(in_channels=1024, out_channels=2048)
        # self.conv_block7 = ConvBlock(in_channels=2048, out_channels=1024)
        # self.conv_block8 = ConvBlock(in_channels=1024, out_channels=512)
        # self.conv_block9 = ConvBlock(in_channels=512, out_channels=256)
        # self.conv_block10 = ConvBlock(in_channels=256, out_channels=256)

        self.fc1 = nn.Linear(43, 768, bias=True)
        self.fc_audioset = nn.Linear(768, classes_num, bias=True)
...
class Wavegram_Logmel128_Cnn14(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        ...
        self.fc1 = nn.Linear(2048, 64, bias=True)
...
class Cnn14_DecisionLevelMax(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
        ...
        self.conv_block3 = ConvBlock(in_channels=128, out_channels=256)
        self.conv_block4 = ConvBlock(in_channels=256, out_channels=512)
        self.conv_block5 = ConvBlock(in_channels=512, out_channels=768)
        # self.conv_block6 = ConvBlock(in_channels=1024, out_channels=2048)
        self.fc0 = nn.Linear(8, 1, bias=True)
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        x = x.transpose(1, 2)
        x = F.relu_(self.fc1(x))
        segmentwise_output = torch.sigmoid(x)

        output_dict = {'embedding': segmentwise_output,
                       'clipwise_output': segmentwise_output}
